#!/usr/bin/env python3
"""
Train an 88-band hyperspectral pavement crack segmentation model.

Input:
  88sampleset/img/*.npy          image cubes, expected shape (88, H, W)
  88sampleset/masknpy/*.npy      binary masks, expected shape (H, W)
  88sampleset/testimg/*.npy      held-out test image cubes
  88sampleset/testmasknpy/*.npy  held-out test masks

Output:
  88output_results/88best_model.pth
  88output_results/checkpoints/last_checkpoint.pth
  88output_results/logs/metrics_log.txt
  88output_results/logs/final_test_metrics.txt
  88output_results/figures/*.png
  88output_results/predictions/predictions_epoch_*.png
"""
...
try:
    ...
    raise SystemExit(
        ...
        "Please install the training dependencies first. For RTX 3090 Ti, install a CUDA-enabled PyTorch build, "
...
class Config:
    ...
    dataset_dir: Path = base_dir / "88sampleset"
    ...
    output_dir: Path = base_dir / "88output_results"
    checkpoint_dir: Path = output_dir / "checkpoints"
    figure_dir: Path = output_dir / "figures"
    prediction_dir: Path = output_dir / "predictions"
    log_dir: Path = output_dir / "logs"
    best_model_path: Path = output_dir / "88best_model.pth"
...
def resolve_dataset_paths(cfg: Config) -> None:
    """Find a dataset root that contains img/masknpy/testimg/testmasknpy.

    AutoDL uploads often rename the 88-band folder. This keeps the script portable
    while still requiring the full leakage-safe train/test directory layout.
    """
    candidates: List[Path] = [
        cfg.dataset_dir,
        cfg.base_dir / "88sampleset",
        cfg.base_dir / "88band",
        cfg.base_dir / "88bands",
        cfg.base_dir / "sampleset_88bands",
        cfg.base_dir,
    ]
    if cfg.base_dir.parent != cfg.base_dir:
        candidates.extend(
            [
                cfg.base_dir.parent / "88sampleset",
                cfg.base_dir.parent / "88band",
                cfg.base_dir.parent / "88bands",
                cfg.base_dir.parent / "sampleset_88bands",
            ]
        )
    ...
    raise FileNotFoundError(
        "Could not find a complete 88-band dataset directory. Expected one folder containing "
...
class DilateAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        output = v * attention
        output = output + self.relu(self.batch_norm(x))
        return output


class MScrackSeg88(nn.Module):
    def __init__(self, in_channel: int = 1, classnum: int = 1, num_bands: int = 88, dim: int = 8) -> None:
        super().__init__()
        self.dim = dim
        ...
        self.transpose1 = nn.ConvTranspose3d(dim, dim, kernel_size=2, stride=2)
        self.conv_up1 = nn.Conv3d(dim, dim, kernel_size=(7, 7, 7), padding=3, stride=1)

        self.transpose2 = nn.ConvTranspose3d(dim * 2, dim, kernel_size=2, stride=2)
        self.conv_up2 = nn.Conv3d(dim, dim, kernel_size=7, padding=3, stride=1)

        self.transpose3 = nn.ConvTranspose3d(dim * 2, dim, kernel_size=2, stride=2)
...
def compute_pos_weight(pairs: Sequence[Tuple[str, Path, Path]], cfg: Config) -> float:
    ...
    return float(np.clip(pos_weight, 1.0, cfg.max_pos_weight))
...
def make_pseudo_rgb(cube: np.ndarray) -> np.ndarray:
    if cube.ndim == 4:
        cube = cube[0]
    bands = cube.shape[0]
    rgb_indices = [min(bands - 1, 12), min(bands - 1, 36), min(bands - 1, 70)]
    rgb = np.stack([normalize_for_display(cube[i]) for i in rgb_indices], axis=-1)
    return rgb
...
def save_prediction_figure(
    ...
    for images, masks, keys in loader:
        ...
        for i in range(images_np.shape[0]):
            rgb = make_pseudo_rgb(images_np[i])
            truth = masks_np[i, 0]
...
def plot_curve(path: Path, title: str, xlabel: str, series: Sequence[Tuple[str, Sequence[float]]]) -> None:
...
def load_checkpoint_if_available(
    ...
    if not cfg.last_checkpoint_path.is_file():
        return 0, -1.0, []
...
def main() -> None:
    ...
    print_dataset_summary(train_pairs, val_pairs, test_pairs)
    ...
    model = MScrackSeg88(in_channel=1, classnum=1, num_bands=cfg.num_bands).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=cfg.learning_rate, weight_decay=cfg.weight_decay)
    scaler = torch.cuda.amp.GradScaler(enabled=cfg.use_amp and device.type == "cuda")
    criterion = CombinedLoss(pos_weight=pos_weight, dice_weight=cfg.dice_loss_weight)

    start_epoch, best_score, history = load_checkpoint_if_available(model, optimizer, scaler, cfg, device)
